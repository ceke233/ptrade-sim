...
纯属浪费：它按**每个交易日十几 MB** 稳定累积，且**不随缓存预算变化** ——
即使把分钟缓存预算压到很小，整机 RSS 仍会一路上涨，直到吃满 DuckDB 的默认上限
（系统内存的 80%）。

设 `memory_limit` 后它会在该上限附近封顶、RSS 随即平稳，
这是长区间回测内存可控的**必要条件**（引擎自己的缓存平稳还不够）。
...
>
> 这不是理论风险：实测同一策略、同一区间，仅因几次被吞掉的查询失败，
> 收益与成交笔数就会出现**量级级别的差异** —— 而两次运行都报告「回测完成」。
...
- `get_Ashares`：返回**沪深全部 A 股**（主板 + 创业板 + 科创板），
  **不含北交所** —— 官方语义是「沪深市场」。按回测日期做**时点**过滤
  （只按 `list_date`/`delist_date` 判定，**不看** `list_status` ——
  后者是「当前」状态，用于历史查询会产生幸存者偏差）
