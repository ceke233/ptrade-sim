...
| 实验 | 结果 | 排除 |
|---|---|---|
| 深测引擎持有的**全部**对象（day50→250） | 只涨 **67 MB**，同期 RSS 涨 3,981 MB | 不是引擎对象图 |
| 分钟预算压到 300 MB | 缓存稳定 296 MB，RSS 仍 19.9 MB/天 | 不是引擎缓存 |
...
| 6 年区间 RSS | **43.7 GB** 且仍在涨 | **平稳 16.4 GB** |
| 900 天处趋势 | 30+ GB 且爬升 | 16.2 → 16.5 GB **基本不动** |

> 16.4 GB 的构成：分钟缓存约 10 GB（预算内、设计如此）+ DuckDB 1.9 GB + 其余约 4 GB。
...
`pool = sorted(get_Ashares())` 得 **5380 只** >> 500，然后每个交易日
