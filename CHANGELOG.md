...
> **本次未修但已定位**：``runtime._stock_status_one`` 的
> ``query_type == "DELISTING"`` 分支（``filter_stock_by_status`` 的实现）有
> **完全相同**的两处问题。实测 ``filter_stock_by_status(codes, ["DELISTING"])``
> 在 2020-01-02 返回 319 只，而当日实际已退市的只有 110 只 ——
> 多出的 209 只是「**未来才退市**」的股票，把它们排除同样构成幸存者偏差。
> 该函数被策略直接调用，修它属于行为变更，待定。
