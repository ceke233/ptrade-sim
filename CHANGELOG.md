...
#### 🔴 `get_stock_info` 泄漏未来：简称与退市日都取了「末尾快照」

与 `get_Ashares` 同一类问题 —— 把「当前状态」当成「历史状态」用。
官方签名 ``get_stock_info(stocks, field=None)`` **没有日期参数**，
但该接口在**回测模块也可用**，所以三个字段都必须反映「回测当日」。

实测某只标的的完整轨迹：

```
002002.SZ   2021 年叫「鸿达兴业」，2023-10-12 起戴帽「ST鸿达」，2024-03-18 退市
stock_basic 里只有末尾快照： name='ST鸿达(退)'  delist_date='20240318'
```

修复前 ``get_stock_info`` 直接回这两个值，于是：

- ``stock_name``：回测到 2021 年就看到 ``ST鸿达(退)`` —— 同时泄漏
  「后来戴帽」与「后来退市」；
- ``de_listed_date``：2021 年就能读到 ``2024-03-18`` —— **等于知道结局**。

更要紧的是**引擎内部两个取名 API 互相矛盾**：``get_stock_name`` 一直是时点
正确的（docstring 写明三层回退、首选当日日线 ``name``），而 ``get_stock_info``
用末尾快照 —— 同一代码同一天报出两个不同的名字。
...
| ``listed_date`` | 静态事实，不变（回测日必然已在市，否则不会出现在池子里）|

**影响面**：本项目 77 个策略里 31 个调用 ``get_stock_info``，但**全部只用
``listed_date``**（过滤新股），另两个字段取了不用 —— 所以现有结果**零影响**。
修它是为了补上隐患：模板里已经把这两个字段取出来了，哪天有人加一句
「按名字过滤 ST」就会静默用上未来信息。

新增 ``tests/test_stock_info_pit.py``（8 项）：简称时点正确、
两个取名 API 同源、退市日不泄漏未来（三个回测日）、已退市给真实日期、
未退市恒为哨兵值、上市日静态、``field=None`` 只回 stock_name、未知代码回 None。
已用变异检验确认有效（简称改回末尾快照 → 3 项失败；
退市日直接回表值 → 1 项失败）。
...
#### 🔴 `get_Ashares` 用 `list_status` 过滤历史日期 → 幸存者偏差

同一方法里还有第二处：过滤条件带了 ``list_status == "L"``。
那是「**当前**是否在市」，用它过滤历史日期就是**幸存者偏差** ——
2020 年在市、2023 年退市的股票现在状态是 ``D``，查 2020 年时会被错误排除。

    实测被误排除的规模随回测起点变早而增大
      2019-01-02  3567 -> 3351   误排 216（6.1%）
      2020-01-02  3760 -> 3553   误排 207（5.5%）
      2024-01-02  5096 -> 5011   误排  85（1.7%）
      2025-01-02  5121 -> 5088   误排  33（0.6%）

被排除的**全是退市股**（退市锐电、乐视退、金亚退…）—— 回测只见幸存者，
会系统性高估收益。

**修法**：删掉 ``list_status``，时点判定只用 ``list_date`` / ``delist_date``
与查询日的比较。实测数据是干净的（``list_status='D'`` 但 ``delist_date`` 为空：
0 行；``list_status='L'`` 却带退市日：0 行），所以这个判定可靠。

#### 🔴 `get_Ashares` 的日期比较混用格式（潜伏 bug）

删掉 ``list_status`` 后暴露出一个更深的：``cur`` 由 ``day_iso()`` 得到
``YYYY-MM-DD``，而库内 ``list_date`` / ``delist_date`` 是 ``YYYYMMDD``。
字符串比较在**同年份**时必然判错（第 5 个字符 ``'0'`` > ``'-'``）：

    '20250301' <= '2025-06-01'  ->  False   （已退市却未排除）
    '20250101' >  '2025-06-01'  ->  True    （已上市却被排除）

即「**当年新上市的股票被错误排除、当年退市的被错误包含**」。
此前被 ``list_status='L'`` 掩盖了大部分症状。

**修法**：查询日改用 ``norm_day()``（YYYYMMDD），并在 polars 里对
``list_date`` / ``delist_date`` 一次性去掉 ``-``，循环内只做同格式比较。
顺带让 ``day_iso()`` 先经 ``norm_day()`` 归一 —— 它此前对已带 ``-`` 的输入
会输出 ``'2025--0-1-02'`` 这种垃圾，且**不报错**，只在下游静默判错。

#### 🔴 `filter_stock_by_status(DELISTING)` 有**完全相同**的两处问题

官方对该接口的定义是「获取**指定日期**证券的 ST、停牌、退市等属性」——
是按时点的。而 ``_stock_status_one`` 的 DELISTING 分支：
``row.get("list_status") == "D"`` **或** ``str(dd) <= day_iso(ds)``。

两者都错：

- ``list_status`` 是「**当前**是否退市」→ 把「**未来才退市**」的股票也判为已退市；
- ``day_iso`` 与库内 ``YYYYMMDD`` 混比 → 同年份判错（同上一条的格式问题）。

    实测 ``filter_stock_by_status(codes, ["DELISTING"])``
      查询日        正确答案(当日已退市)   现状返回   多判
      2020-01-02              110            319    +209
      2024-01-02              234            319     +85
      2025-01-02              286            319     +33
      2025-06-03              299            319     +20

多判出来的正是「**未来才退市**」的股票（退市锐电、乐视退…）——
策略据此在历史回测里提前剔除未来的输家，**同样是幸存者偏差**。
这个函数被策略每天调用（``filter_stock_by_status(pool, ("ST","HALT","DELISTING"))``），
影响面比 ``get_Ashares`` 更直接。

**修法**：只按 ``delist_date <= 查询日`` 判定（同为 YYYYMMDD 比较），
不看 ``list_status``；无退市日即未退市。

> 顺带确认了同函数另外两支**本来就是正确的**：``ST`` 取当日日线的 ``is_st``、
> ``HALT`` 取当日分钟量 —— 都是时点数据，无需改动。

新增 ``tests/test_status_delisting.py``（6 项）：退市日前查询为未退市、
退市日及之后为已退市、**同年份退市必须查得出**（格式回归）、
未退市恒为 False、``list_status`` 不得参与判定（防回退）、
``filter_stock_by_status`` 端到端只剔掉当时已退市的。
已用变异检验确认有效（注入 ``list_status=='D'`` + ``day_iso`` 混比 → 4 项失败）。

新增 ``tests/test_get_ashares.py``（13 项）锁住：纳入沪深三板块、
排除北交所、**退市股在退市前必须返回**（幸存者偏差回归）、
同年份退市必须被排除（日期格式回归）、未上市/已退市时点过滤、
``list_status`` 不得出现在过滤条件（防回退）、脏数据被排除并告警、
新号段被暴露、告警去重、``day_iso`` 容错。
已用变异检验确认有效（回退为「只主板」→ 4 项失败；
加回 ``list_status`` → 6 项失败）。
